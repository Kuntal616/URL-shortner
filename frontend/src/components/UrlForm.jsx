...
const UrlForm = () => {
  ...
  const handleSubmit = async() => {
    ...
    // }
     setShortUrl('');
    try {
    const {data} = await axios.post('http://localhost:3000/api/shorturl', { url });
    setShortUrl(data.shortURL);
  } catch (error) {
    // Handle axios error (status 400, 500, etc.)
    if (error.response && error.response.data && error.response.data.error) {
      setError(error.response.data.error);
    } else {
      setError('Something went wrong. Please try again.');
    }
     setShortUrl(''); // Only clear shortUrl when there's an error
  }
  }
  ...
  return (
    <div className='space-y-4'>
      ...
      {submitted && error && <p className='text-red-500 text-sm mt-2'>{error}</p>}
      {submitted && shortUrl && (
        <p className='text-green-500 text-sm mt-2'>URL shortened successfully!</p>
      )}
      <button
        type='submit'
